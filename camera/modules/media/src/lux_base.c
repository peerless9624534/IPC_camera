...
int Show_Policy_Max_Min_Priority(int policy)
{
    ...
    if (priority == -1)
    {
        printf("%s:%d sched_get_priority_max error!\n", __func__, __LINE__);
...
{
    p_thrdschd->threadId = pthread_self();
    printf("threadId=%lu\n", p_thrdschd->threadId);
    struct sched_param thdsched;
    if (0 != pthread_getschedparam(p_thrdschd->threadId, &(p_thrdschd->policy), &thdsched))
    {
        printf("%s:%d pthread_getschedparam error!\n", __func__, __LINE__);
        return BASE_COMM_ERR;
    }
    p_thrdschd->priority = thdsched.sched_priority;
    printf("threadPolicy=%d\n", p_thrdschd->policy);
...
{
    struct sched_param thdsched;
    thrdschd.threadId = pthread_self();
    thdsched.sched_priority = thrdschd.priority;
    return pthread_setschedparam(thrdschd.threadId, thrdschd.policy, &thdsched);
}
...
int SetThread_RR_Prio(int priority)
{
    struct sched_param thdsched;
    ThrdSchd thrdschd;
    thrdschd.policy = SCHED_RR;
    thrdschd.threadId = pthread_self();
    thdsched.sched_priority = priority;
    return pthread_setschedparam(thrdschd.threadId, thrdschd.policy, &thdsched);
}

/**
 * @description: 创建一个线程,设置线程调度策略及优先级，并设置分离属性
...
int TaskPolicy_Prio_CreateThread(void* func, void* arg, ThrdSchd thrdschd)
{
    ...
    if (0 != pthread_create(&thrdschd.threadId, NULL, func, arg))
    {
    ...
    /* 设置线程分离属性 */
    pthread_detach(thrdschd.threadId);

    if (0 != SetThread_Policy_Prio(thrdschd))
    {
        printf("%s:%d SetThread_Policy_Prio error!\n", __func__, __LINE__);
        return BASE_COMM_ERR;
    }
...
int TaskRR_Prio_CreateThread(void* func, void* arg, int priority)
{
    ThrdSchd thrdschd;
    thrdschd.policy = SCHED_RR;
    thrdschd.priority = priority;
    return TaskPolicy_Prio_CreateThread(func, arg, thrdschd);
}
