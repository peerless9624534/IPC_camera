...
static LUX_DayNight_INFO_ST g_DayNightInfo =
{
    ...
    .lowThreshold = 300,
    .highThreshold = 350,
    .dayNightMod = LUX_DAYNIGHT_DAY_MODE,
...
INT_X LUX_DayNight_SetAutoMode(LUX_DayNight_AutoMode_EN autoMode)
{
    ...
    if (LUX_DAYNIGHT_AUTO_MODE == autoMode
        && LUX_DAYNIGHT_NIGHT_MODE == g_DayNightInfo.dayNightMod)
    {
...
//                }
            ...
            /*开放出来给硬件测试调试*/
            LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, "daynight", "cntmax", &cntMax);
 ...
 */
static INT_X LUX_DayNight_SetSartMod(PCHAR_X startMod)
{
    INT_X ret = LUX_ERR;
...
INT_X LUX_DayNight_Init()
{
    ...
    /* 配置文件读取白天黑夜切换的阈值 */
    LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, "daynight", "lowthreshold", &g_DayNightInfo.lowThreshold);
    LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, "daynight", "highThreshold", &g_DayNightInfo.highThreshold);
    ...
    /*根据涂鸦配置选择daynight模式*/
    ret = LUX_INIParse_GetString(TUYA_INI_CONFIG_FILE_NAME, "video", "daynight" , startMod);
    if(0 != ret)
    {
        IMP_LOG_ERR(TAG, "LUX_INIParse_GetBool failed!, error num [0x%x] ", ret);
    }

    ret = LUX_DayNight_SetSartMod(startMod);
    if (LUX_OK != ret)
    {
