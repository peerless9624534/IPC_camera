...
#define LUX_AI_CRY_SECTION "function"
...
#define LUX_SLEEP_MODE   "sleep_mode"
...
static INT_X LUX_AUDIO_ReadIni()
{
    ...
    /* COMMON */
    ret = LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AUDIO_IN_TYPE, (int*)&pAiAttr->chnAttr.payloadType);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AUDIO_IN_TYPE failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AUDIO_IN_SAMPLE_RATE, (int *)&pAiAttr->impAttr.samplerate);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AUDIO_IN_SAMPLE_RATE failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AUDIO_OUT_TYPE, (int *)&pAoAttr->chnAttr.payloadType);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AUDIO_OUT_TYPE failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AUDIO_OUT_SAMPLE_RATE, (int *)&pAoAttr->impAttr.samplerate);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AUDIO_OUT_SAMPLE_RATE failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetBool(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AUDIO_AEC, &g_audioAttr.bAecEnable);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AUDIO_AEC failed,error [0x%x]\n", ret);

    /* AUDIO IN */
    ret = LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AI_VOLUME, &pAiAttr->chnAttr.volume);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AI_VOLUME failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AI_GAIN, &pAiAttr->chnAttr.gain);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AI_GAIN failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetBool(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AI_NS, &pAiAttr->chnAttr.bNsEnable);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AI_NS failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AI_NS_LENEL, (int*)&pAiAttr->chnAttr.nsLevel);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AI_NS_LENEL failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetBool(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AI_HPF, &pAiAttr->chnAttr.bHpfEnable);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AI_HPF failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetBool(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AI_AGC, &pAiAttr->chnAttr.bAgcEnable);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AI_AGC failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AI_AGC_LEVEL, &pAiAttr->chnAttr.agcParam.targetLevelDbfs);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AI_AGC_LEVEL failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AI_AGC_GAIN, &pAiAttr->chnAttr.agcParam.compressionGaindB);
    if (LUX_OK != ret)
    ...
    //#ifdef BABY_PRO
    INT_X value = 0;
    ret = LUX_INIParse_GetInt(TUYA_INI_CONFIG_FILE_NAME, "function", LUX_AO_VOLUME, &value);
    if ((LUX_OK != ret) || (value > 10))
    {
        value = 9;
        char tmp[16] = {'\0'};
        sprintf(tmp, "%d", value);
        ret = LUX_INIParse_SetKey(TUYA_INI_CONFIG_FILE_NAME, "function", LUX_AO_VOLUME, tmp);
        if (LUX_OK != ret)
        {
    ...
    //#else
    // ret = LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AO_VOLUME, &pAoAttr->chnAttr.volume);
    // if (LUX_OK != ret)
    //     IMP_LOG_ERR(TAG, "LUX_AO_VOLUME failed,error [0x%x]\n", ret);
    //#endif
    
    ret = LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AO_GAIN, &pAoAttr->chnAttr.gain);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AO_GAIN failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetBool(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AO_HPF, &pAoAttr->chnAttr.bHpfEnable);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AO_HPF failed,error [0x%x]\n", ret);
    
    ret = LUX_INIParse_GetBool(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AO_AGC, &pAoAttr->chnAttr.bAgcEnable);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AO_AGC failed,error [0x%x]\n", ret);
    
    ret = LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AO_AGC_LEVEL, &pAoAttr->chnAttr.agcParam.targetLevelDbfs);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AO_AGC_LEVEL failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetInt(START_INI_CONFIG_FILE_NAME, LUX_AUDIO, LUX_AO_AGC_GAIN, &pAoAttr->chnAttr.agcParam.compressionGaindB);
    if (LUX_OK != ret)
        IMP_LOG_ERR(TAG, "LUX_AO_AGC_GAIN failed,error [0x%x]\n", ret);

    ret = LUX_INIParse_GetBool(TUYA_INI_CONFIG_FILE_NAME, LUX_AI_CRY_SECTION, LUX_SLEEP_MODE, &sleepMode);
    if (LUX_OK != ret)
    IMP_LOG_ERR(TAG, "cry_onoff failed,error [0x%x]\n", ret);

    if(LUX_FALSE == sleepMode)
    {
        ret = LUX_INIParse_GetBool(TUYA_INI_CONFIG_FILE_NAME, LUX_AI_CRY_SECTION, LUX_AI_CRY_ONOFF, &pCry->bCryOpen);
        if (LUX_OK != ret)
            IMP_LOG_ERR(TAG, "cry_onoff failed,error [0x%x]\n", ret);
...
INT_X LUX_AI_GetStream(LUX_AUDIO_FRAME_ST *pFrame)
{
    ...
    do
    {
        ...
        if (NULL == aiFp)
        {
            aiFp = fopen("/tmp/audio_in_1.pcm", "w+");
        }
        if (NULL != aiFp)
        {
...
BOOL_X LUX_AI_GetCryDet(void)
{
    ...
    do
    {
        ret = LUX_INIParse_GetBool(TUYA_INI_CONFIG_FILE_NAME, LUX_AI_CRY_SECTION, LUX_AI_CRY_ONOFF, &bOpen);
        if (LUX_OK != ret)
        {
            IMP_LOG_ERR(TAG, "cry_onoff failed,error [0x%x]\n", ret);
            break;
...
extern FLOAT_T g_cryDetectSensitive; /* 哭声检测灵敏度 */
extern LUX_IVS_TIMEINTERVAL_EN g_alarmInterval; /* 报警时间间隔 */
extern BOOL_T g_bcryDetctAlarmSong;
extern BOOL_T g_pwmBlubEnable;
...
void LUX_AI_CryDet_Process()
{
    ...
    while (1)
    {
        ...
        do
        {
            ...
            {
                if (TUYA_APP_IsAlarmByInterval(lastTime)) /* unnit:s */
                {
                    g_cryAttr.cryCount++;
                    ...
                    /* 抓图成功，上报婴儿哭声报警信息 */
                    IPC_APP_report_sound_alarm("");
                    tuya_ipc_notify_with_event(capPic[LUX_STREAM_SUB].pAddr, capPic[LUX_STREAM_SUB].streamLen, NOTIFICATION_CONTENT_JPEG, NOTIFICATION_NAME_BABY_CRY);
                    ...
                    if (SS_WRITE_MODE_EVENT == tuya_ipc_ss_get_write_mode() )
                    {
                        ...
                        /* 创建本地录像处理定时器 */
                        memset(&timerParam_event, 0, sizeof(timerParam_event));
                        timerParam_event.bTimeStartEnable = LUX_TRUE;
                        timerParam_event.timeStartValue = LUX_COMM_EVENT_RECORD_TIME;
                        timerParam_event.pTimerProcess = (void *)LUX_stopCryEventRecordCB;

                        ssTimerId = LUX_BASE_TimerCreate(&timerParam_event);
                    ...
                    {
                        event_id = tuya_ipc_cloud_storage_event_add(capPic[LUX_STREAM_SUB].pAddr, capPic[LUX_STREAM_SUB].streamLen, 0, LUX_CLOUD_STORAGE_TIME);
...
INT_X LUX_Customed_Voice_Action(void)
{
    ...
    do
    {
        ret = LUX_INIParse_GetInt(TUYA_INI_CONFIG_FILE_NAME, LUX_AI_CRY_SECTION, LUX_CUSTOMED_VOICE_ACTION, &iAction);
        if (LUX_OK != ret)
        {
            IMP_LOG_ERR(TAG, "get custom_voice_action failed,error [0x%x]\n", ret);
            break;
        }
    } while (0);
