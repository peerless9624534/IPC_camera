...
{
    p_out->video_codec = (TUYA_CODEC_VIDEO_H265 == s_media_info.video_codec[E_CHANNEL_VIDEO_MAIN])
                            ? TUYA_CODEC_VIDEO_H265 : TUYA_CODEC_VIDEO_H264;
    //printf("%s:%d >>>>>>>>> p_out->video_codec:%d \n\n", __FUNCTION__, __LINE__, p_out->video_codec);
...
STATIC INT_T __TUYA_APP_p2p_event_cb(IN CONST TRANSFER_EVENT_E event, IN CONST PVOID_T args)
{
    ...
    switch (event)
    {
        case TRANS_LIVE_VIDEO_START:
        {
            ...
            LUX_HAL_LedSetStatus(LUX_HAL_LED_GREEN, LUX_HAL_LED_3_HZ);
        ...
        case TRANS_PLAYBACK_QUERY_DAY_TS:
        {
            ...
            if (OPRT_OK != ret)
            {
                PR_ERR("channel:%d pb_ts query by day: %d-%d-%d Fail", pquery->channel, pquery->year, pquery->month, pquery->day);
                break;
            }
            if (p_day_ts){
...
static TUYA_IPC_TRANSFER_VAR_S p2p_var;
...
STATIC VOID __TUYA_APP_rev_audio_cb(IN CONST TRANSFER_AUDIO_FRAME_S *p_audio_frame, IN CONST UINT_T frame_no)
{
    ...
    TUYA_APP_Rev_Audio_CB(&audio_frame, p2p_var.audio_sample, p2p_var.audio_databits, p2p_var.audio_channel);
}
...
OPERATE_RET TUYA_APP_Enable_P2PTransfer(IN UINT_T max_users)
{
    ...
    p2p_var.audio_sample = (TUYA_AUDIO_SAMPLE_E)audioParam.sampleRate;      /* �˴��޸�ao��Ƶ������ */
    p2p_var.audio_databits = (TUYA_AUDIO_DATABITS_E)audioParam.bitWidth;
    p2p_var.audio_channel = TUYA_AUDIO_CHANNEL_MONO;

    PR_DEBUG("P2P: audio_codec:%d", p2p_var.rev_audio_codec);
    PR_DEBUG("P2P: audio_sample:%d", p2p_var.audio_sample);
    PR_DEBUG("P2P: audio_databits:%d", p2p_var.audio_databits);
    PR_DEBUG("P2P: audio_channel:%d", p2p_var.audio_channel);
