...
//#define SD_MOUNT_PATH           "/system/sdcard"    /* for 1.6.x*/
#define SD_MOUNT_PATH           "/mnt/sdcard"    /* for 1.7.x*/
...
VOID tuya_ipc_sd_format(VOID)
{
    ...
    snprintf(format_cmd, 256, "fuser -m %s;umount %s;mkfs.vfat -n %s %s -s 1024;mkdir -p %s;mount -t auto %s %s;",
             SD_MOUNT_PATH,s_mmcblk_name, SD_MOUNT_PATH, s_mmcblk_name, SD_MOUNT_PATH, s_mmcblk_name, SD_MOUNT_PATH);
    TYDEBUG("execute: %s\n",format_cmd);
...
VOID tuya_ipc_sd_remount(VOID)
{
    ...
    snprintf(format_cmd, 128, "umount %s;sleep 1;mount -t auto %s %s;", s_mmcblk_name, s_mmcblk_name, SD_MOUNT_PATH);
    FILE *pp = popen(format_cmd, "r");
    if(NULL != pp)
    {
        fgets(buffer,sizeof(buffer),pp);
...
E_SD_STATUS tuya_ipc_sd_get_status(VOID)
{
    ...
    if(fp)
    {
        ...
        fclose(fp);
        CHAR_T *mmcblk_name_start = strstr(s_mounts_info, LINUX_SD_DEV_FILE "p1"); //Confirm the final node name in Mount information
...
#define MAX_EVENT_NUM_PER_DAY   (1440)
