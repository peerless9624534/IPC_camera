...
OPERATE_RET hwl_wf_station_connect(IN CONST CHAR_T *ssid,IN CONST CHAR_T *passwd)
{
    ...
    char tmpCmd[128] = {0};
    char ssidTmp[34] = {0}; //32个有效字节
    char pswdTmp[34] = {0}; //32个有效字节
    FILE *fp = NULL;
    ...
    else
    {
        ...
        sprintf(ssidTmp, "%s", ssid);
        sprintf(pswdTmp, "%s", passwd);
        ...
        sprintf(tmpCmd, "udhcpc -i %s -q", WLAN_DEV); /* -q:获取ok后退出*/
        exec_cmd(tmpCmd);
    ...
    while (fgets(tmp, sizeof(tmp), pp) != NULL)
    {
...

        {
            ...
            if(pSSIDStart != NULL)
            {
                ...
                s_aps[recordIdx].s_len = strlen((PCHAR_T)s_aps[recordIdx].ssid);
...
extern void QR_Thread_Exit();

static pthread_t sniffer_thId; // ID of capture thread
static pthread_t qrcode_thId; // ID of capture thread
...
OPERATE_RET hwl_wf_get_ip(IN CONST WF_IF_E wf,OUT NW_IP_S *ip)
{
    ...
    if(wf == WF_AP)
    {
        ...
        memcpy(ip->ip, "192.168.1.10", strlen("192.168.1.10"));
        memcpy(ip->gw, "192.168.1.1", strlen("192.168.1.1"));
...
OPERATE_RET hwl_wf_station_get_conn_ap_rssi(OUT SCHAR_T *rssi)
{
    ...
    *rssi = 0;
    ...
    {
        /* find signal  */
        char *pSIGNALStart = strstr(tmp, "level=");
        if(pSIGNALStart != NULL)
        {
            int x = 0;
            int y = 0;
            sscanf(pSIGNALStart + strlen("level="), "%d/%d", &x, &y);
            *rssi = x * 100/ (y+1);
            break;
        }
    }
...
/* 设置ap的ssid */
static int lux_hwl_wf_set_ap_ssid()
{
    char tmp[256] = { 0 };
    char ssidStr[32] = { 0 };
    ...
    /* 新的ssid写入配置文件 */
    FILE* fp = fopen("/system/init/hostapd.conf", "r+");
    if (NULL == fp)
    {
        printf("%s:%d open [/system/init/hostapd.conf] error!\n", __FUNCTION__, __LINE__);
        return -1;
    }

    sprintf(ssidStr, "ssid=SmartLife-%02X%02X\n", x5, x6);
    printf("%s TUYA_AP_SSID ssidStr:%s\n", __FUNCTION__, ssidStr);

    memset(tmp, 0, sizeof(tmp));
    while (fgets(tmp, sizeof(tmp), fp) != NULL)
    {
        /* find signal  */
        char* pSSIDStart = strstr(tmp, "ssid=");
        if (pSSIDStart != NULL)
        {
...
OPERATE_RET hwl_wf_ap_start(IN CONST WF_AP_CFG_IF_S *cfg)
{
    ...
    /* 修改默认热点名 */
    if (0 == lux_hwl_wf_set_ap_ssid())
    {
        /* 开放AP热点 */
        exec_cmd("/system/bin/hostapd.sh");
    }
    else
    {
        printf("lux_hwl_wf_set_ap_ssid error!\r\n");
    }
