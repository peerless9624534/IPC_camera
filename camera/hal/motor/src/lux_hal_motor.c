...
static int LUX_HAL_Motor_CtrlMov(LUX_HAL_MOTOR_DIRECTION_EN direction, int yDistance, int xDistance)
{
    ...
    if(x != g_motorAttr.x)
    {
        ...
        ret = LUX_INIParse_SetKey(TUYA_INI_CONFIG_FILE_NAME, "function", "MOTOR_X", tmp);
        if (LUX_OK != ret)
        {
    ...
    if(y != g_motorAttr.y)
    {
        ...
        ret = LUX_INIParse_SetKey(TUYA_INI_CONFIG_FILE_NAME, "function", "MOTOR_Y", tmp);
        if (LUX_OK != ret)
        {
            IMP_LOG_ERR(TAG, "ERROR:LUX_INIParse_SetKey ret:%#x", ret);
            return -1;
...
int LUX_HAL_Motor_CtrlMovOneStep(LUX_HAL_MOTOR_MOV_DIRECTION_EN direction)
{
    ...
    do{
        /*在隐私模式下不进行电机转动*/
        LUX_INIParse_GetInt(TUYA_INI_CONFIG_FILE_NAME, "function", "sleep_mode", &sleepMode);
        if (1 == sleepMode)
        {
...
{       
    ...
    else if(LUX_HAL_MOTOR_CRUISE_TIMING == cruiseMode)
    {
        ...
        LUX_INIParse_GetString(TUYA_INI_CONFIG_FILE_NAME, "motor", "patrol_time_start", tStartBuf);
        ...
        LUX_INIParse_GetString(TUYA_INI_CONFIG_FILE_NAME, "motor", "patrol_time_end", tEndBbuf);
...
int LUX_HAL_MOTOR_CruiseModeConfig(void)
{
    ...
    LUX_INIParse_GetInt(TUYA_INI_CONFIG_FILE_NAME, "function", "sleep_mode", &sleepMode);
    LUX_INIParse_GetInt(TUYA_INI_CONFIG_FILE_NAME, "motor", "patrol_switch",&patrolSwitch);
    LUX_INIParse_GetInt(TUYA_INI_CONFIG_FILE_NAME, "motor", "patrol_tmode", &patrolTMode);
    LUX_INIParse_GetInt(TUYA_INI_CONFIG_FILE_NAME, "motor", "patrol_mode", &patrolMod);
...
int LUX_HAL_Motor_CtrlInit()
{
    ...
    int ret = -1;
    //获取电机位置
    ret = LUX_INIParse_GetInt(TUYA_INI_CONFIG_FILE_NAME, "function", "MOTOR_X", &g_motorAttr.x);
    if (LUX_OK != ret)
    {
    ...
    ret = LUX_INIParse_GetInt(TUYA_INI_CONFIG_FILE_NAME, "function", "MOTOR_Y", &g_motorAttr.y);
    if (LUX_OK != ret)
    {
